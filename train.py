...
LOCAL_RANK = int(os.getenv('LOCAL_RANK', -1))  # 这个Worker是这台机器上的第几个Worker
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    '''
    # ----------------------------------------------- 初始化参数和配置信息 ----------------------------------------------
    # saver_dir:保持训练结果路径,如runs/train/exp18
    save_dir, epochs, batch_size, weights, single_cls, evolve, data, cfg, resume, noval, nosave, workers, freeze, = \
        Path(opt.save_dir), opt.epochs, opt.batch_size, opt.weights, opt.single_cls, opt.evolve, opt.data, opt.cfg, \
        opt.resume, opt.noval, opt.nosave, opt.workers, opt.freeze

    # w:保存权重路径,如runs/train/exp18/weights
    w = save_dir / 'weights'
    (w.parent if evolve else w).mkdir(parents=True, exist_ok=True)  # make dir
    last, best = w / 'last.pt', w / 'DSDebris.pt'   # runs/train/exp18/weights/last.pt # runs/train/exp18/weights/DSDebris.pt

    # Hyperparameters
    if isinstance(hyp, str):
        with open(hyp, errors='ignore') as f:
            hyp = yaml.safe_load(f)  # 加载超参数
    #日志输出超参数信息 hyperparameters: ...
    ...
    # 将模型参数分为三组(bn, weights, biases)来进行分组优化
    ...
    if RANK in [-1, 0]:
        ...
        if not resume:
            # 统计dataset的label信息
            # labels: [11748,5] 数据集中有11748个target [:, class+x+y+w+h]
            labels = np.concatenate(dataset.labels, 0)
            # c = torch.tensor(labels[:, 0])  # classes 将class类别转成tensor
...
def parse_opt(known=False):
    ...
    parser.add_argument('--weights', type=str, default=ROOT / 'DSDebris.pt', help='initial weights path')
    parser.add_argument('--cfg', type=str, default='models/debris.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default=ROOT / 'data/debris.yaml', help='dataset.yaml path')
