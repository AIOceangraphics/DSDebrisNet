...
def run(weights=ROOT / 'DSDebris.pt',  # 事先训练完成的权重文件，比如yolov5s.pt,假如使用官方训练好的文件（比如yolov5s）,则会自动下载
        source=ROOT / 'data/images',  # 预测时的输入数据，可以是文件/路径/URL/glob, 输入是0的话调用摄像头作为输入
        imgsz=640,  # 预测时的放缩后图片大小
        ...
        save_conf=False,  # labels 是否将结果中的置信度保存在txt文件中
        save_crop=False,  # 是否保存裁剪后的预测框
        nosave=False,  # 是否保存预测后的图片/视频
        classes=None,  # 过滤指定类的预测结果 filter by class: --class 0, or --class 0 2 3
        ...
        exist_ok=False,  # 推理结果覆盖之前的结果 False: 推理结果新建文件夹保存,文件夹名递增
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    if save_txt:  # Write to file
                        ...
                        # line = (cls, *xywh, conf) if save_conf else (cls, *xywh)  # label format ，保存的成的txt文件有xywh
                        line = (cls,conf,frame) if save_conf else (cls,frame)  # label format ，保存的成的txt文件没有xywh
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'DSDebris.pt', help='model path(s)')
    parser.add_argument('--source', type=str, default=ROOT / 'data/images/0000046.jpg', help='file/dir/URL/glob, 0 for webcam')
